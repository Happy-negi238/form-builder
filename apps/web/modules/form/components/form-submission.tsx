...
import { useFingerprint } from "~/hooks/fingerpring"
...
const getFormErrorMessage = (message?: string) => {
  switch (message) {
    case "Not any form exist":
      return "This form could not be found."
    case "Form is not publish yet":
      return "This form is not available yet."
    case "From is expired":
      return "This form is closed because it has expired."
    case "Form submission limit exceed":
      return "This form is no longer accepting responses."
    default:
      return message || "This form is unavailable right now."
  }
}
...
type PrivateFormProps = {
  ...
  setPasswordToVerify: (password: string) => void
  passwordError: string
  setPasswordError: (error: string) => void
  isPasswordCheckError: boolean
  isPasswordChecking: boolean
}

const handleIsPrivateForm = ({
    ...
    <Dialog open={isPasswordDialogOpen} onOpenChange={(open) => open && setIsPasswordDialogOpen(true)}>
      ...
      >
        ...
        <form onSubmit={handleVerifyPassword} className="space-y-4">
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPasswordChecking} className="h-11 w-full bg-zinc-300 font-semibold text-zinc-950 hover:bg-zinc-200">
              {isPasswordChecking ? "Checking password..." : "Submit password"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
...
const FormSubmission = ({ formId }: FormSubmissionProps) => {
  ...
  const { getFromByIdData, isPending, isError, error } = useGetFromById(formId, fingerPrint)
  ...
  const {
    formSubmissionAsync,
    isPending: isFormSubmissionPending,
  ...
  const hasAlreadySubmitted =
    error?.message === "You have already submitted this form" ||
    formSubmissionError?.message === "You have already submitted this form"
  ...
  if (isError || !getFromByIdData) {
    return (
      <main className="grid min-h-screen place-items-center bg-zinc-950 p-6 text-center text-zinc-400">
        {getFormErrorMessage(error?.message)}
      </main>
    )
  }
  ...
  const onSubmit = async (formValues: SubmissionValues) => {
    ...
    try {
      const { formSubmissionId } = await formSubmissionAsync({ formId, values: submissionValues, fingerPrint })
      console.log("formSubmissionId: ", formSubmissionId)
      setSubmitted(true)
    } catch {
      // The mutation error is rendered from formSubmissionError below.
    }
  }
              ...
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  {isFormSubmissionError && (
                    <p className="text-sm text-red-400">
                      {getFormErrorMessage(formSubmissionError?.message)}
                    </p>
                  )}
